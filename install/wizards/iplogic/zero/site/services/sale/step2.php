...
if(Option::get("zero", "wizard_installed", "N", WIZARD_SITE_ID) != "Y")
{
	$locationGroupID = 0;
	$arLocationArr = Array();
	...
	else
	{
		...
		else
		{
			...
			if($bRus)
				$groupLang[] = array("LID" => $lang, "NAME" => Loc::getMessage("SALE_WIZARD_GROUP"));
	...
	if(!$dlvBySiteExist)
	{
		$deliveryItems[] = array(
			...
			"SORT" => 100,
			"ACTIVE" => /*$delivery["courier"] == "Y" ? "Y" : "N",*/"Y",
		...
		$deliveryItems[] = array(
			"NAME" => Loc::getMessage("SALE_WIZARD_COUR1"),
			"DESCRIPTION" => Loc::getMessage("SALE_WIZARD_COUR1_DESCR"),
			"CLASS_NAME" => '\Bitrix\Sale\Delivery\Services\Configurable',
			"CURRENCY" => $defCurrency,
			"SORT" => 200,
			"ACTIVE" => /*$delivery["self"] == "Y" ? "Y" : "N",*/"Y",
...
if(CModule::IncludeModule('subscribe'))
{
	...
	if(!file_exists($template))
	{
		...
		if(file_exists($fname) && is_file($fname) && ($fh = fopen($fname, "rb")))
		{
			...
			$php_source = str_replace("Hello!", Loc::getMessage("SUBSCR_1"), $php_source);
	...
	if(!$rsRubric->Fetch())
	{
		...
		$arFields = Array(
			...
			"NAME"		=> Loc::getMessage("SUBSCR_1"),
			"SORT"		=> 100,
			"DESCRIPTION"	=> Loc::getMessage("SUBSCR_2"),
			"LID"		=> WIZARD_SITE_ID,
			"AUTO"		=> "Y",
			"DAYS_OF_MONTH"	=> "",
			...
			"VISIBLE"	=> "Y",
			"FROM_FIELD"	=> Option::get("main", "email_from", "info@ourtestsite.com"),
			"LAST_EXECUTED"	=> ConvertTimeStamp(false, "FULL"), 
		);
		$obRubric = new CRubric;
		$ID = $obRubric->Add($arFields);
	}
	Option::set('subscribe', 'subscribe_section', '#SITE_DIR#personal/subscribe/');
}
...
else
{
	...
	$arFields = Array(
		"ACTIVE"       => "Y",
		"C_SORT"       => 300,
		"NAME"         => Loc::getMessage("SALE_WIZARD_CONTENT_EDITOR"),
		"DESCRIPTION"  => Loc::getMessage("SALE_WIZARD_CONTENT_EDITOR_DESCR"),
		"USER_ID"      => array(),
...
if(IntVal($userGroupID) > 0)
{
	...
	WizardServices::SetFilePermission(Array($siteID, $SiteDir . "/auth/"), Array($userGroupID => "W"));
	WizardServices::SetFilePermission(Array($siteID, $SiteDir . "/cart/"), Array($userGroupID => "W"));
...
Option::set("zero", "wizard_installed", "Y", false, WIZARD_SITE_ID);
?>