...
class SelectTemplateStep extends CSelectTemplateWizardStep
{
	...
	function ShowStep()
	{
		...
		foreach ($arTemplateOrder as $ID) {
			$preview = false;
			if (file_exists($_SERVER["DOCUMENT_ROOT"].$wizard->GetPath()."/site/templates/".$ID."/images/".LANGUAGE_ID."/preview.gif"))
				$preview = $wizard->GetPath()."/site/templates/".$ID."/images/".LANGUAGE_ID."/preview.gif";
			elseif (file_exists($_SERVER["DOCUMENT_ROOT"].$wizard->GetPath()."/site/templates/".$ID."/images/preview.gif"))
		...
		foreach ($arTemplateInfo as $arTemplate)
		{
			...
			$this->content .= $this->ShowRadioField("wizTemplateID", $arTemplate["ID"], Array("id" => $arTemplate["ID"], "class" => "inst-template-list-inp"));


			$this->content .= '<label for="'.$arTemplate["ID"].'" class="inst-template-list-label" style="width:100%">';
			$this->content .= CFile::ShowImage($arTemplate["PREVIEW"], 100, 100, 'class="inst-template-list-img" style="border:1px solid black;"', "", false);
		...
		$this->content .= 	'<div class="wizard-input-form">
									<div class="wizard-input-form-block">
										<label for="templateName" class="wizard-input-title">'.Loc::getMessage('WIZ_TEMPLATE_NAME').'</label><br>
										'.$this->ShowInputField('text', 'templateName', ["id" => "templateName", "class" => "wizard-field"]).'
									</div>
									<div class="wizard-input-form-block">
										<label for="templateDescription" class="wizard-input-title">'.Loc::getMessage('WIZ_TEMPLATE_DESCRIPTION').'</label><br>
										'.$this->ShowInputField('text', 'templateDescription', ["id" => "templateDescription", "class" => "wizard-field"]).'
									</div>
									<div class="wizard-input-form-block">
										<label for="templateDir" class="wizard-input-title">'.Loc::getMessage('WIZ_TEMPLATE_DIR').'</label><br>
